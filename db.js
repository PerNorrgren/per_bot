...
async function getDb() {
  ...
  // Per's request — free-preview limits and end-of-preview popup content,
  // both configurable per media type, with the same "override falls back
  // to the level above" inheritance as file_sequence_override elsewhere
  // in this app (global → course → lesson → this one file, in this one
  // lesson). One row per (scope, media type) that's actually been
  // customised; a level with no row at all just isn't customised there,
  // and getEffectivePreviewConfig below walks up to whichever ancestor
  // level does have one, ultimately falling back to hardcoded defaults
  // if even the global level has never been set. scope_id is NULL only
  // for scope_type='global' — every other scope_type always has a real
  // id. media_type is 'audio' for now; video/pdf/ebook are the same
  // shape, just not built yet, so no schema change needed to add them
  // later. limit_value's unit depends on media_type (seconds for audio/
  // video, pages for pdf, chapters for ebook) — deliberately not itself
  // stored, since it's implied by media_type and would just be one more
  // thing that could drift out of sync with it.
  db.run(`CREATE TABLE IF NOT EXISTS preview_configs (
    id TEXT PRIMARY KEY,
  ...
  db.run(`CREATE UNIQUE INDEX IF NOT EXISTS idx_preview_configs_scope
    ON preview_configs(scope_type, COALESCE(scope_id,''), media_type)`);
  ...
  const migrations = [
    ...
    // Per's request — audio free-preview should only play the first 3
    // minutes (the whole thing, if it's already shorter), not the full
    // file. NULL means "no trimmed clip needed/generated yet" — either
    // the file is short enough that the original already serves fine as
    // its own preview, or ensureFreePreviewClip (server.js) hasn't run
    // for it yet; a real R2 key here means a trimmed copy exists and the
    // free-preview playback-url path should serve that instead of the
    // original. Only ever set for audio files — video/PDF free previews
    // are untouched, still full access, same as before.
    "ALTER TABLE library_files ADD COLUMN preview_key TEXT",
    // Per's request — free preview limits are now configurable per
    // course/lesson/file (preview_configs, see the table below), not a
    // single hardcoded 180s for every file everywhere. Since the same
    // underlying file can be free-previewed from more than one lesson
    // with a different effective limit in each, the cached clip has to
    // know which limit it was actually trimmed to — this is that record.
    // ensureFreePreviewClip regenerates whenever the resolved limit for
    // the current request doesn't match what's cached, rather than
    // trusting a stale clip from a different context's limit.
    "ALTER TABLE library_files ADD COLUMN preview_key_seconds INTEGER",
  ];
...
// preview_key cleared here — a trimmed free-preview clip made from the
// OLD audio would otherwise keep serving after the content underneath
// it changed. ensureFreePreviewClip (server.js) regenerates it lazily,
...
function replaceLibraryFileContent(id, filename, fileType, fileSize, originalName, storageType, originalPdfFilename) {
  getDbSync().run('UPDATE library_files SET filename=?, file_type=?, file_size=?, original_name=?, storage_type=?, original_pdf_filename=?, preview_key=NULL, preview_key_seconds=NULL WHERE id=?',
...
// Per's request — the trimmed clip an audio file's free preview actually
// serves, generated by ensureFreePreviewClip in server.js. seconds is
// what it was actually trimmed to (see preview_key_seconds migration
// note above) — ensureFreePreviewClip compares this against the
// currently-resolved limit to decide whether the cached clip is still
// valid or needs regenerating. NULL/null clears both back to "no
// trimmed clip" (used when the file's underlying content is replaced —
// see replaceLibraryFileContent above, a stale trim from the old audio
// would otherwise keep serving).
function setLibraryFilePreviewKey(fileId, key, seconds) {
  getDbSync().run('UPDATE library_files SET preview_key=?, preview_key_seconds=? WHERE id=?', [key, seconds ?? null, fileId]); save();
}

// ── Preview configs (free-preview limits + end-of-preview popup) ──
// Per's request — a genuinely new feature, not an extension of anything
// that already existed: how long a free preview runs, and what the
// popup says when it ends, are both configurable at four levels
// (global/course/lesson/file_ref) with the same override-falls-back-to-
// parent inheritance already proven by file_sequence_override elsewhere
// in this app. scope_id is null only for scope_type='global'.
function getPreviewConfig(scopeType, scopeId, mediaType) {
  return queryOne(
    'SELECT * FROM preview_configs WHERE scope_type=? AND COALESCE(scope_id,\'\')=COALESCE(?,\'\') AND media_type=?',
    [scopeType, scopeId ?? null, mediaType]
  );
}
// Upserts by hand (query-then-insert-or-update) rather than leaning on
// the unique index's own ON CONFLICT, since better-sqlite/sql.js's
// UPSERT support on an expression index (COALESCE(scope_id,'')) isn't
// worth the fragility here — this table is only ever written from a
// handful of admin actions, never a hot path.
function setPreviewConfig(scopeType, scopeId, mediaType, fields) {
  const existing = getPreviewConfig(scopeType, scopeId, mediaType);
  const { limitValue, headline, body, buttonLabel, buttonUrl } = fields;
  if (existing) {
    getDbSync().run(
      'UPDATE preview_configs SET limit_value=?, headline=?, body=?, button_label=?, button_url=? WHERE id=?',
      [limitValue ?? null, headline ?? null, body ?? null, buttonLabel ?? null, buttonUrl ?? null, existing.id]
    );
  } else {
    getDbSync().run(
      'INSERT INTO preview_configs (id,scope_type,scope_id,media_type,limit_value,headline,body,button_label,button_url) VALUES (?,?,?,?,?,?,?,?,?)',
      [crypto.randomUUID(), scopeType, scopeId ?? null, mediaType, limitValue ?? null, headline ?? null, body ?? null, buttonLabel ?? null, buttonUrl ?? null]
    );
  }
  save();
}
// Removes a level's override entirely, so it goes back to inheriting
// from its parent — distinct from setPreviewConfig with blank fields,
// which would instead save an override that happens to be blank.
function deletePreviewConfig(scopeType, scopeId, mediaType) {
  getDbSync().run(
    'DELETE FROM preview_configs WHERE scope_type=? AND COALESCE(scope_id,\'\')=COALESCE(?,\'\') AND media_type=?',
    [scopeType, scopeId ?? null, mediaType]
  );
  save();
}
// Ultimate fallback if even the global level has never been set — so a
// fresh install (or a media_type nobody's configured yet) still has a
...
// as a single unit (a course-level override doesn't mix-and-match
// fields with the global one; whichever level answers first is used
// whole, same reasoning file_sequence_override already uses: simpler to
// build, simpler to reason about, and simpler for Per to see exactly
// what's in effect at each level without field-by-field bookkeeping).
// Any of courseId/lessonId/fileRefId can be omitted — a context that
// doesn't apply (there's no file_ref because this isn't inside a
// lesson at all, say) just skips that level and keeps walking up.
function getEffectivePreviewConfig(mediaType, { courseId, lessonId, fileRefId } = {}) {
  const levels = [
  ...
  for (const [scopeType, scopeId] of levels) {
    const row = getPreviewConfig(scopeType, scopeId, mediaType);
    if (row) return { source: scopeType, limitValue: row.limit_value, headline: row.headline, body: row.body, buttonLabel: row.button_label, buttonUrl: row.button_url };
  }
  const d = PREVIEW_CONFIG_DEFAULTS[mediaType] || {};
  return { source: 'default', limitValue: d.limitValue ?? null, headline: d.headline ?? null, body: d.body ?? null, buttonLabel: d.buttonLabel ?? null, buttonUrl: d.buttonUrl ?? null };
}
...
module.exports = {
  ...
  setLessonFileRefFreePreview, getFreePreviewRef, setLibraryFilePreviewKey,
  getPreviewConfig, setPreviewConfig, deletePreviewConfig, getEffectivePreviewConfig,
