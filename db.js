...
function replaceLibraryFileContent(id, filename, fileType, fileSize, originalName, storageType, originalPdfFilename) {
  getDbSync().run('UPDATE library_files SET filename=?, file_type=?, file_size=?, original_name=?, storage_type=?, original_pdf_filename=?, preview_key=NULL WHERE id=?',
...
// Per's request — the trimmed (first-3-minutes) clip an audio file's free
// preview actually serves, generated by ensureFreePreviewClip in
// server.js. NULL clears it back to "no trimmed clip" (used when the
// file's underlying content is replaced — see replaceLibraryFileContent
// above, a stale trim from the old audio would otherwise keep serving).
function setLibraryFilePreviewKey(fileId, key) {
  getDbSync().run('UPDATE library_files SET preview_key=? WHERE id=?', [key, fileId]); save();
}
